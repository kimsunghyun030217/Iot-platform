...
OpenCV를 활용하여 카메라 영상에서 학생의 얼굴과 눈 영역을 검출하고,  
눈의 상태 변화를 기반으로 졸음 여부를 판단합니다.

졸음이 감지되면 해당 학생의 의자에 설치된 진동 장치를 작동시켜  
개별적으로 졸음을 깨울 수 있도록 하였습니다.

또한 전체 수업 인원의 **1/3 이상에서 졸음이 감지될 경우**  
교실 환경에도 문제가 있다고 판단하여  
서보모터를 이용해 창문을 자동으로 개방하도록 구현했습니다.
...
이를 통해 특정 학생의 졸음에는 **개별 진동 알림**으로 대응하고,  
...
OpenCV 기반 졸음 감지와 IoT 하드웨어 제어를 결합하여  
학생 개인에게는 즉각적인 피드백을 제공하고,  
다수의 학생에게 동시에 졸음이 발생하는 경우에는  
교실 환경까지 자동으로 조절하는 스마트 교실 시스템을 구현했습니다.
